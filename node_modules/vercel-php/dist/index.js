...
async function build({ files, entrypoint, workPath, config = {}, meta = {}, }) {
    ...
    const userFiles = await build_utils_1.download(files, workPath, meta);
    ...
    if (userFiles[COMPOSER_FILE]) {
        ...
        await utils_1.runComposerInstall(workPath);
        // Run composer scripts (created files are collected bellow, , see harvestedFiles)
        await utils_1.runComposerScripts(userFiles[COMPOSER_FILE], workPath);
    }
    // Append PHP directives into php.ini
    if (userFiles['api/php.ini']) {
        const phpini = await utils_1.modifyPhpIni(userFiles, runtimeFiles);
        if (phpini) {
    ...
    const harverstedFiles = build_utils_1.rename(await build_utils_1.glob('**', {
        cwd: workPath,
    ...
    if (process.env.NOW_PHP_DEBUG === '1') {
        ...
        console.log('🐘 PHP: php.ini', await utils_1.readRuntimeFile(runtimeFiles['php/php.ini']));
    }
